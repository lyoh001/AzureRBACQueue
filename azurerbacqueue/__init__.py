...
def main(msg: func.ServiceBusMessage):
    ...
    logging.info(f"Subscription Name: {(subscription := msg.get_body().decode('utf-8').split(','))[0]}")
    ...
    try:
        upns = "\n".join(requests.get(url=f"https://management.azure.com{rbac['properties']['roleDefinitionId']}?api-version=2015-07-01", headers=rest_api_headers).json()["properties"]["roleName"] + f" Role: {response.json()['userPrincipalName']} has been added to AAD Guest RBAC Review. " + str(requests.post(url="https://graph.microsoft.com/v1.0/groups/c6f8666e-053a-4f09-a15c-6feee253af06/members/$ref", headers=graph_api_headers, json={"@odata.id": f"https://graph.microsoft.com/v1.0/directoryObjects/{rbac['properties']['principalId']}"}).status_code) for rbac in rbacs if (response := requests.get(url=f"https://graph.microsoft.com/v1.0/users/{rbac['properties']['principalId']}", headers=graph_api_headers)).status_code == 200 and "#EXT#" in response.json()["userPrincipalName"])
        logging.info((file_data := f"Subscription Name: {subscription[0]}\nSubscription Id: {subscription[1]}\nUPNs:\n{upns}\n\n\n\n"))
        logging.info("-------------------------------------------------------------------------------------")
